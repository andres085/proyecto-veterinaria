...
export const useDuenioStore = defineStore('duenio', () => {
  ...
  const fetchAll = async (): Promise<void> => {
    try {
      ...
      duenios.value = response.data || response || []
      console.log(`✅ ${duenios.value.length} dueños cargados`)
      
    } catch (err: any) {
      error.value = err.response?.data?.message || err.message || 'Error al cargar dueños'
  ...
  const fetchOne = async (id: number): Promise<Duenio | null> => {
    try {
      ...
      error.value = err.response?.data?.message || err.message || 'Error al cargar dueño'
  ...
  const create = async (data: CreateDuenioPayload): Promise<Duenio | null> => {
    try {
      ...
      error.value = err.response?.data?.message || err.message || 'Error al crear dueño'
   ...
   */
  const update = async (id: number, data: UpdateDuenioPayload): Promise<Duenio | null> => {
    try {
      loading.value = true
      error.value = null
      ...
      const updatedDuenio = response.data || response
      ...
      error.value = err.response?.data?.message || err.message || 'Error al actualizar dueño'
  ...
  const remove = async (id: number): Promise<boolean> => {
    try {
      ...
      error.value = err.response?.data?.message || err.message || 'Error al eliminar dueño'
  ...
  const search = async (query: string): Promise<void> => {
    try {
      ...
      error.value = err.response?.data?.message || err.message || 'Error en la búsqueda'
  ...
  const filterLocal = computed(() => (query: string): Duenio[] => {
    ...
    return duenios.value.filter(duenio => 
